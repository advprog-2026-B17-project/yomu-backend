package id.ac.ui.cs.advprog.yomubackend.read_quiz.repository;

import id.ac.ui.cs.advprog.yomubackend.read_quiz.model.Answer;
import id.ac.ui.cs.advprog.yomubackend.read_quiz.model.Grading;
import org.springframework.data.jpa.repository.JpaRepository;
import java.util.List;

public interface GradingRepository extends JpaRepository<Grading, Long> {
    List<Grading> findByAnswer(Answer answer);
    List<Grading> findByAnswerQuizAttemptId(Long attemptId);
}
